...
\begin{flushleft}
    ...
    2: $Col(A) \subseteq \mathbb{R}^{3}, Col(A^{T}) \subseteq \mathbb{R}^{2}$ $(Rank = min(rows_n,cols_n) \land \text{列之间线性无关})$ \\
    ...
    $Col(A^{T})$ = $\begin{bmatrix}
            ...
            a_{21} & a_{22} & a_{23} \\
        \end{bmatrix}$
    已知道 $Rank(Col(A^{T})) = 2$ 那么 $Col(A^{T})$ 的列必然存在一组线性相关 我们设$col1$与$col2$线性无关, 那么 $Col(A^{T})$的基底组是: $\begin{bmatrix}
            a_{11} & a_{12} \\
            a_{21} & a_{22} \\
        \end{bmatrix}$
    ...
    \end{align}
    我们发现 e 残差向量被去掉了 得到一个 $z  = A^{T}p$  z 是内积标量向量 是一个占比的向量在 等式的一边 检查 $\hat{c}$ 是不是 这个等式的解
