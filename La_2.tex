...
\begin{flushleft}
    ...
    2: $A^{T}$ 与 $A$  \\
    % 我们知道 $A \in \mathbb{R}^{3 \times 2}$ (三行二列) \\
    % $A^{T} \in \mathbb{R}^{2 \times 3}$ \\
    % 对于 A 而且 输入和输出是确定 它的 $ in \in \mathbb{R}^{2}  \to  out \in \mathbb{R}^{3}$,
    % 这是一个标准的升维映射
    % 而 $A^{T}$ 它的 $ in \in \mathbb{R}^{3}  \to  out \in \mathbb{R}^{2}$,
    % 这是一个标准的降维映射
